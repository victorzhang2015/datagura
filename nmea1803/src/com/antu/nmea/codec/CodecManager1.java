 ...
 */
public class CodecManager1 extends Observable implements Observer {
	...
	public void decode(Date timestamp, char[] content) {
		...
		CodecManager1.logger.debug("received: " + new String(content));
		...
		for (String rawSentence : rawSentences) {
			...
			CodecManager1.logger.debug("decoding sentence: " + rawSentence);
			...
			if (checkSumIndex >= 0) {
				if (!this.isChecksumValid(rawSentence)) {
					CodecManager1.logger.info("sentence checksum validation failed.");
					continue;
				}
				CodecManager1.logger.info("sentence checksum validation passed.");
			}
			...
			CodecManager1.logger.info("sentence segmented.");

			try {
				// proprietary sentence
				if (rawSentence.startsWith("$P") || rawSentence.startsWith("!P")) {
					CodecManager1.logger.info("processing as proprietary sentence.");
					
					String sentenceType = segments[0].substring(2);
					
					if (sentenceType == null || sentenceType.length() < 3) {
						CodecManager1.logger.error("cannot determine sentence type for: " + rawSentence);
						continue;
					}
					
					sentenceType = sentenceType.substring(0, 3).toLowerCase();
						
					CodecManager1.logger.debug("trying to resolve sentence type: " + sentenceType);
					
					if (this.acceptanceList.shouldAcceptProprietary(sentenceType)) {
						CodecManager1.logger.info("sentenceType: " + sentenceType + " is accepted.");
						...
						if (codec != null) {
							...
							CodecManager1.logger.error("unable to find proprietary codec for sentence type: " + sentenceType);
						}
					} else {
						CodecManager1.logger.info("sentenceType: " + sentenceType + " is not in acceptance list.");
					}
				} else if (segments[0].startsWith("$") &&
						   segments[0].length() == 6 && 
						   segments[0].charAt(5) == 'Q') { // query sentence
					CodecManager1.logger.info("processing as query sentence.");
					...
					if (segments[0].length() == 6) {
						...
						if (this.acceptanceList.shouldAcceptPublic(sentenceType)) {
							CodecManager1.logger.info("sentence type accepted.");
							
							// parametric sentence
							if (segments[0].startsWith("$")) {
								CodecManager1.logger.info("processing as parametric/multi-sentence sentence.");
								// first check to see if it's a multisentence
								if (MultiSentenceTable.instance().isMultiSentenceType(sentenceType)) {
									CodecManager1.logger.info("processing as multi-sentence sentence.");
									...
									if (codec != null) {
										codec.addObserver(this);
										CodecManager1.logger.info("codec located.");
										codec.decode(timestamp, sentenceType, segments);
									} else {
										CodecManager1.logger.error("unable to find multi-sentence codec for sentence type: " + sentenceType);
									}
								} else { // no, do as a regular parametric sentence.
									CodecManager1.logger.info("processing as parametric sentence.");
									this.parametricSentenceCodec.decode(timestamp, sentenceType, segments);
								}
							}
							else if (segments[0].startsWith("!")) { // encapsulation sentence
								CodecManager1.logger.info("processing as encapsulation sentence.");
								...
								if (codec != null) {
									codec.addObserver(this);
									CodecManager1.logger.info("codec for encapsulation sentence located.");
									codec.decode(timestamp, sentenceType, segments);
								} else {
									CodecManager1.logger.error("unable to find encapsulation sentence codec for sentence type: " + sentenceType);
								}
							}
						} else {
							CodecManager1.logger.info("sentence type: " + sentenceType + " not accepted.");
						}
					}
				}
				...
				CodecManager1.logger.error("error parsing sentence: " + rawSentence, e);
			}
		}
	}
	...
	public void update(Observable o, Object arg) {
		...
		CodecManager1.logger.info("parsed object received: " + arg.toString());
	...
	public List<String> encode(String talker, INmeaSentence sentence) {
		...
		try {
			...
			if (MultiSentenceTable.instance().isMultiSentenceType(sentence.sentenceType())) {
				...
				CodecManager1.logger.error("encoding multi-sentence sentence, type: " + sentence.sentenceType());
				...
				CodecManager1.logger.error("encoding parametric sentence, type: " + sentence.sentenceType());
				...
				CodecManager1.logger.error("encoding encapsulation sentence, type: " + sentence.sentenceType());
				...
				CodecManager1.logger.error("encoding query sentence, type: " + sentence.sentenceType());
			...
			CodecManager1.logger.error("exception occurred during encoding.", e);
			return new ArrayList<String>();
		}
	}
}
