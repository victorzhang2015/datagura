...
public class CodecFactory<E> {
	...
	static private Log logger = LogFactory.getLog(CodecFactory.class);

	@SuppressWarnings("rawtypes")
	public CodecFactory(String pattern) {
	...
	@SuppressWarnings("rawtypes")
	public CodecFactory(String defaultPackage, String pattern) {
	...
	protected Class<?> findClass(String className) {
		...
		try {
			CodecFactory.logger.debug("class found: " + className);
			Class<?> clz = Class.forName(className);
			this.initializedClasses.put(className, clz);
			return clz;
		} catch (ClassNotFoundException e) {
			CodecFactory.logger.fatal("class: " + className + " cannot be found", e);
			return null;
		}
	}
	...
	public E getByFullName(String className) 
		...
		if (this.initializedObjects.containsKey(className)) {
			CodecFactory.logger.info("cached object found");
			...
			CodecFactory.logger.info("object not in cache, initialized");
			return obj;
		} else {
			Class<?> clz = this.findClass(className);
