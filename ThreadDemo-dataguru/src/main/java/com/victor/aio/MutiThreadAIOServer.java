...
public class MutiThreadAIOServer {
    ...
    public MutiThreadAIOServer() throws IOException{
        //设置线程数为CPU核数
        ...
        //绑定端口并设置连接请求队列长度
        server.bind(new InetSocketAddress(8000), 80);    
    }
    ...
    private void writeBuffer(final AsynchronousSocketChannel channel, ByteBuffer buffer) {
        channel.write(buffer, buffer, new CompletionHandler<Integer, ByteBuffer>() {
            ...
            public void completed(Integer result, ByteBuffer buffer) {
            	System.out.println(Thread.currentThread().getName() + ": run in write completed method" ); 
